...
from blockstore.apps.bundles.store import (
    Dependency, DraftRepo, Link, LinkCollection, SnapshotRepo, is_safe_file_path
)
        ...
        def to_representation(self, value):
            ...
            basic_info['links'] = {
                link.name: {
                    ...
                    "modified": link.name in staged_draft.links_to_overwrite.names_to_links
                }
...
class DraftFileUpdateSerializer(serializers.BaseSerializer):
    ...
    def _parse_files(self, files):
        for file_name in files:
            if not is_safe_file_path(file_name):
                raise ValidationError(f'"{file_name}" is not a valid file name')
    ...
    def _parse_links(self, links):
        ...
        1. Verify that the linked Bundles actually exist.
        2. Verify that we have permission to use them (punting for now).
        3. Find all the indirect dependencies of links.
        4. Create a LinkCollection.
        """
        ...
        for name, bv_info in links.items():
            ...
            if not bundle_version:
                raise ValidationError(
                    (
                        f"BundleVersion ({bundle_uuid}, {version}) referenced "
                        f"in Link {name} does not exist."
                    )
                )

            # If everything checks out, create a Dependency. We can't make a
            # Link yet because we don't know the indirect
